...
def create_order(customer_id: str, items: list[dict]) -> dict:
    """Create a customer order. Safe to retry — uses idempotency key."""
    # Generate a deterministic key from the operation parameters
    # so retries of the same logical operation use the same key
    idempotency_key = str(uuid.uuid5(
        uuid.NAMESPACE_OID,
        f"create_order:{customer_id}:{sorted(str(i) for i in items)}"
    ))
...
The idempotency key should be deterministic from the input — if you retry because of a timeout, you want to reuse the same key so the server recognises the duplicate. Using `uuid.uuid4()` (random) would generate a new key on every retry, defeating the purpose.
