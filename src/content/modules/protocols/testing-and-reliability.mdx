...
class TestGetCustomerOrder:
    ...
    def test_handles_not_found(self, mock_api):
        ...
        result = get_customer_order("ORD-INVALID")
        assert "error" in result
        assert result["order_id"] == "ORD-INVALID"

    def test_rejects_invalid_order_id_format(self):
        # Schema validation should catch this before the API call
...
class MockTool:
    ...
    def __init__(self, name: str, responses: list | Callable):
        self.name = name
...
def make_test_tools():
    search_mock = MockTool("search_knowledge_base", responses=[
        '{"results": [{"id": "doc-1", "text": "Refunds take 5–7 business days."}]}',
        '{"results": []}',  # Second call returns empty
    ])
    return {search_mock.name: search_mock}, [search_mock.as_schema()]

# Test that the model calls the right tool
def test_refund_query_calls_search():
    ...
    assert any(tc.name == "search_knowledge_base" for tc in response.tool_calls)
    assert "refund" in tools_dict["search_knowledge_base"].calls[0].get("query", "").lower()
```
