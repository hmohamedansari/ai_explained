...
This system prompt works with any provider — you're just defining a contract for the model's output. The JSON format itself is universal; only the SDK call wrapping it changes.
...
- Use structured output / function calling to constrain the output shape — a valid JSON schema cannot be overridden by injected text
...
- Prompt engineering guides — each major provider publishes one for their models: [Anthropic](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview) · [OpenAI](https://platform.openai.com/docs/guides/prompt-engineering) · [Google](https://ai.google.dev/gemini-api/docs/prompting-strategies). The techniques overlap heavily; read any one of them.
