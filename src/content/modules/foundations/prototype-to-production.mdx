...
**Error handling**
- [ ] Application handles model refusals gracefully (stop_reason = "end_turn" with no useful content)
...
- [ ] Batch API considered for high-volume offline tasks (typically 50% cost reduction at the expense of latency)
...
def ask(question):
    response = client.messages.create(
        model="claude-opus-4-6",      # frontier for everything
        max_tokens=4096,              # generous global default
        messages=[{"role": "user", "content": question}]
    )
    return response.content[0].text  # raw string, no validation
```
...
def ask(question: str, history: list[dict]) -> str:
    ...
    response = client.messages.create(
        model="claude-haiku-4-5-20251001",  # right-sized for this task
...
- [Building effective agents — Anthropic](https://www.anthropic.com/research/building-effective-agents) — Practical guide to production agent architecture from Anthropic; most principles apply to non-agent AI features too.
