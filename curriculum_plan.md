# AI Academy: Curriculum Plan
> **Living document.** Last substantive revision: 2026-03-20.
...
- **UX Pattern:** Progressive Disclosure — see Section 7 for the full content spec and schema mapping.
...
- **Role-Based Filtering:** UI allows users to "Choose their Path" on the landing page.
...
| 1.4 | Prompting Fundamentals | All | System/user/assistant turns, few-shot, chain-of-thought, zero-shot, ReAct pattern |
| 1.5 | Sampling Parameters | All → Dev | Temperature, Top-P, Top-K, frequency/presence penalties — what each knob actually controls and when to touch it |
| 1.6 | Structured Outputs | Jr Dev → Sr Dev | JSON mode, Pydantic/Instructor, why structured > string parsing |
| 1.7 | Model Families & Trade-offs | Leader / Dev | Frontier vs. open-weight, parameter count, capability vs. cost vs. privacy |
| 1.8 | Quantisation & Local Models | Dev / SRE | GGUF format, INT4/INT8/5-bit, VRAM as hard constraint, GPU offloading, why small quantised models structurally fail at multi-step tool use |
| 1.9 | DSPy: Programmatic Prompt Optimisation | Sr Dev | Moving past manual prompting, declarative LM programs, compiled prompts |
| 1.10 | Multimodal AI | All Devs | Vision models (GPT-4o, Claude, LLaVA/LLaVA-NeXT), audio transcription (Whisper, Parakeet), video understanding; how multimodal inputs change the context engineering problem; when to use vision vs. OCR |
...
| 2.2 | Vector Stores & Indexing | Jr Dev → | Chroma (local), Qdrant/Pinecone/Weaviate/Milvus (production), what an index actually is, insert/query patterns, hosted vs. self-hosted tradeoffs — the conceptual bridge before building a RAG pipeline |
...
| 2.7 | GraphRAG & Knowledge Graphs | Sr Dev | Neo4j, FalkorDB, entity-relationship extraction, when graphs beat vectors |
...
| 3.2 | Skills: Atomic Agent Capabilities | Jr Dev → | Composable skill units, separation of capability from orchestration |
| 3.3 | Model Context Protocol (MCP) | All Devs | The n×m integration problem, MCP as a standard, tool discovery, Linux Foundation ownership |
| 3.4 | Agent2Agent (A2A) | Sr Dev | How agents discover and communicate with other agents via capability cards at well-known URLs — analogous to DNS/service discovery |
| 3.5 | AG-UI: Streaming Protocol | Sr Dev | SSE (Server-Sent Events) standard abstracting framework-specific event formats from the frontend |
| 3.6 | Agent Payments Protocol (AP2) | Leader / Sr Dev | Cryptographic mandate chains for agent spending authorisation, audit trails, approval workflows — an entirely new infrastructure category |
| 3.7 | LiteLLM: API Standardisation | Jr Dev / SRE | Unified API proxy across 100+ providers, model routing, fallbacks, cost tracking |
| 3.8 | Curated Tool Selection | Sr Dev | **Why fewer tools outperform large tool catalogs** — agents get confused by too many options; curation is an architectural decision |
| 3.9 | AGENTS.md Convention | Sr Dev | Repo-level machine-readable config encoding codebase-specific rules for AI agents — emerging standard in engineering orgs |
...
| # | Module | Personas | Key Concepts |
|---|---|---|---|
| 4.1 | What Is an Agent? (Actually) | All | The "agent as coworker" vs. "agent as chatbot" paradigm shift; the Plan → Act → Observe loop |
...
| 4.4 | AutoGen: Conversational Multi-Agent | Jr Dev → | Microsoft's framework for multi-agent conversations, group chat patterns |
| 4.5 | Multi-Agent Failure Modes | Sr Dev | The "bag of agents" anti-pattern, implicit state assumptions, the **17× error trap** (3 agents at 90% accuracy = 73% system accuracy) |
| 4.6 | Typed Contracts at Agent Boundaries | Sr Dev | **Discriminated union / action schema pattern** — typed action schemas eliminate open-ended agent output ambiguity; machine-checkable contracts stop bad state propagating |
| 4.7 | Subagent Orchestration | Sr Dev | Decomposing complex tasks, supervisor vs. peer architectures, hierarchical delegation |
| 4.8 | Sandbox Isolation | Sr Dev / SRE | Modal, Daytona, Runloop — purpose-built ephemeral compute for agents with full permissions; not just Docker |
| 4.9 | Cognitive Architectures | Sr Dev | Memory + planning + tool loops; ReAct/Plan-and-Execute/Reflexion pattern family; long-term memory and persistent state |
| 4.10 | Internal Coding Agents | Sr Dev / Leader | Slack-first invocation, integrating with Linear/GitHub/CI, rich startup context, AGENTS.md + sandbox stack |
| 4.11 | Middleware & Deterministic Injection | Sr Dev | Hybrid architecture mixing rule-based guardrails with LLM reasoning — reliability without losing flexibility |
...
| 5.8 | LLM Gateway Patterns | SRE | LiteLLM as gateway, load balancing, model fallbacks, cost ceilings, RBAC for prompt access |
...
| 6.8 | Observability Stack | All Devs | Langfuse (open-source tracing + prompt management), LangSmith (LangGraph debugging), Arize Phoenix (embedding visualisation), Helicone (proxy-based cost/latency) — trace-level vs. log-level visibility |
| 6.9 | Cost Management | SRE / Leader | Token counting, prompt caching, cost ceilings, billing models for multi-agent systems |
| 6.10 | Evaluating the Evaluator | Sr Dev | Production monitoring that validates your evals themselves remain trustworthy — the meta-loop most teams skip |
...
| 7.6 | PII Scrubbing & Data Privacy | All Devs / Leader | What data enters the context window, PII detection and redaction, data residency, sovereign AI options |
| 7.7 | Red Teaming Methodology | Sr Dev / Leader | Garak, DeepTeam, refusal-aware red teaming, adversarial testing requirements under EU AI Act, building a repeatable red team process |
| 7.8 | Kill Switches & Human-in-the-Loop | Sr Dev / SRE | Mandatory intervention points for high-stakes tool calls, approval workflows, graceful degradation |
| 7.9 | EU AI Act & GRC | Leader | Risk classification tiers, prohibited uses, compliance obligations, audit requirements, the timeline; bias detection and hallucination monitoring |
| 7.10 | Constitutional AI & RLHF | Sr Dev | Anthropic's Constitutional AI approach, RLHF mechanics, how safety is baked in at training time vs. applied at inference |
...
| 8.3 | Buy vs. Build vs. Tune | Leader | Decision matrix, total cost of ownership, open source vs. proprietary, when fine-tuning is actually the answer, when it isn't |
...
| Discriminated union / action schema pattern | Eliminates open-ended agent output ambiguity | 4.6 |
| The 17× error trap | System accuracy = product of each agent's accuracy | 4.5 |
| Sandbox isolation providers | Modal/Daytona/Runloop — purpose-built, not just Docker | 4.8 |
| AGENTS.md convention | Machine-readable repo config for AI agents | 3.9, 4.10 |
| Agent Payments Protocol (AP2) | Cryptographic authorisation for agentic commerce | 3.6 |
| Capability cards / A2A discovery | Agent equivalent of DNS | 3.4 |
...
## 10. Interactive Labs (Future Scope)

*Scoped conservatively. Each lab must justify its maintenance cost. Revisit scope after initial content proves valuable.*
